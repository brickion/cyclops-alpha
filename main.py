...
RECOGNITION_THRESHOLD = 0.62 # looser recognition is 0.65, model is trained with 0.6
...
def main_loop():
    ...
    while True:
        ...
        for (top, right, bottom, left), face_label in zip(face_locations, face_labels):
            ...
            cv2.putText(frame, face_label, (left + 6, bottom - 6), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 255), 1)
        ...
        for metadata in known_face_metadata:
            ...
            if datetime.now() - metadata["last_seen"] < timedelta(seconds=10) and metadata["seen_frames"] > 5:
                ...
                x_position = number_of_recent_visitors * 80
                frame[30:110, x_position:x_position + 80] = metadata["face_image"]
                number_of_recent_visitors += 1

                # Label the image with how many times they have visited
                visits = metadata['seen_count']
                visit_label = f"{visits}th v"
                if visits == 1:
                    visit_label = "1st v"
                cv2.putText(frame, visit_label, (x_position + 10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 255), 1)

        if number_of_recent_visitors > 0:
            cv2.putText(frame, "Person at site", (5, 18), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 255), 1)
